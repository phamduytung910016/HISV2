...
public class PositionController {
    ...
    public ResponseEntity<ResponseObject> getAll() {
        return jobPositionRepository.findAll().size() > 0 ? ResponseEntity.status(HttpStatus.OK).body(
                ResponseObject.builder().status("ok").message("Find all ").data(jobPositionRepository.findAll()).build()) :
                ResponseEntity.status(HttpStatus.NOT_FOUND).body(
                        ResponseObject.builder().status("failed").message("Cannot find ").data(jobPositionRepository.findAll()).build())
                ;
    }
